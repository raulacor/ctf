...
def initial_recon(target, full_url):
    print("Running whois. . .")
    os.system(f"whois {target} >> whois.txt")

    print("Searching web directories with ffuf. . .")
    os.system(f"ffuf -u {full_url}/FUZZ -w /usr/share/wordlists/dirb/common.txt >> ffuf.txt")
...
def main():
    ...
    setup()
    initial_recon(target, full_url)
    subfinders(target, pwd)
